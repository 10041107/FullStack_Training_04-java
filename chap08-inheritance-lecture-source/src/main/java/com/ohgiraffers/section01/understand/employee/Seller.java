...
public class Seller {
    ...
    * 입력받은 타입의 재정이된 price를 호출하여 하나의 매서드로 판매 가격을 책정할 수 있다.
    * 만약 위와 같이 하지 않는 경우 아래의 메서드 oldCarSale(),electricCarSale() 각각
    ...
    * */
    public void sale(Car[] cars){
        ...
        // 이를 통해 다형성의 이점을 누릴수 있게 된다.
        for (Car car:cars) {
            if(car instanceof ElectricCar){ // heap의 생성된 car의 자료형이 ElectriceCar와 같은지를 비교함
                ElectricCar electricCar = (ElectricCar) car; // 여기도
                System.out.println(electricCar.getBrand() + "사의 " + car.getName()+"를 판매하였습니다.");
    ...
    * */
    public void oldCarSale(OldCar[] oldCar){
        for (OldCar car: oldCar) {
            ...
            // 판매 수수료는 판매 가의 10% 이기 때문에 0.1을 곱한다.
